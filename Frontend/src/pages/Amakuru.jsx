...
import LoadingPage from "../components/LoadingPage"; // ✅ spinner
...
const Amakuru = () => {
  ...
  useEffect(() => {
    fetch("https://ihurironews.onrender.com/api/news")
      ...
      .then((data) => {
        setArticles(data);
        setLoadingNews(false); // ✅ stop loading once data arrives
      })
      .catch((err) => {
        console.error("Error fetching articles:", err);
        setLoadingNews(false); // ✅ stop loading even on error
      });
  }, []);

  if (loadingNews) return <LoadingPage />; // ✅ show loading screen
            ...
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              ...
              <div className="space-y-4">
                ...
                {announcements.map((item, index) => (
                  <div
                    key={index}
                    className="flex items-start p-3 bg-white rounded-lg shadow-xs hover:bg-gray-50"
                  >
